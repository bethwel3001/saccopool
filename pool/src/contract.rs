...
/// SaccoChain member-facing stats for a single reserve asset.
///
/// `deposit` and `borrow` are denominated in the underlying asset.
/// `available_to_borrow` is the additional amount the member is allowed to borrow under the
/// SaccoChain rule that a member may borrow at most 2x their current deposit in the same asset
/// (a simple micro-lending cap on top of Blend's health-factor checks).
#[derive(Clone)]
#[contracttype]
pub struct MemberStats {
    pub deposit: i128,
    pub borrow: i128,
    pub available_to_borrow: i128,
}
...
pub trait Pool {
    ...
    /// SaccoChain view: fetch the member-facing stats for `address` on the given `asset`.
    ///
    /// Returns deposit (collateral + non-collateral supply, in underlying asset), current borrow
    /// (in underlying asset), and the amount still available to borrow under the SaccoChain
    /// 2x-deposit micro-lending cap. Note: Blend health-factor checks still apply at borrow time;
    /// this cap is an additional product-level guard rail.
    ///
    /// ### Arguments
    /// * `address` - The member to look up
    /// * `asset` - The reserve asset (e.g. testnet USDC)
    fn get_member_stats(e: Env, address: Address, asset: Address) -> MemberStats;
...
impl Pool for PoolContract {
    ...
    fn get_member_stats(e: Env, address: Address, asset: Address) -> MemberStats {
        ...
        let collateral_b = positions.collateral.get(reserve.config.index).unwrap_or(0);
        let supply_b = positions.supply.get(reserve.config.index).unwrap_or(0);
        let liability_d = positions.liabilities.get(reserve.config.index).unwrap_or(0);

        let collateral_assets = reserve.to_asset_from_b_token(&e, collateral_b);
        let supply_assets = reserve.to_asset_from_b_token(&e, supply_b);
        let deposit = collateral_assets + supply_assets;
        let borrow = reserve.to_asset_from_d_token(&e, liability_d);

        // SaccoChain product rule: a member may borrow up to 2x their current deposit in the
        // same asset. This is on top of (not instead of) Blend's standard health checks.
        let max_borrow = deposit.saturating_mul(2);
        let available_to_borrow = if max_borrow > borrow {
            max_borrow - borrow
        } else {
            0
        };
